...
;; https://github.com/ezzoueidi.com/dot-emacs/-/blob/master/init.org
...
(global-set-key (kbd "C-c g n") 'multi-vterm-next)
(global-set-key (kbd "C-c g p") 'multi-vterm-prev)
...
;(defun ez/unset-proxy ()
;  (interactive)
;  (customize-set-variable 'url-proxy-services nil))

;; No blinking and beeing, no startup screen, no scratch message and short
;; Yes/No questions.
(blink-cursor-mode -1)
...
(setq user-mail-address (or (getenv "EMAIL") "naeil@ezzoueidi.com"))
...
(cond ((eq system-type 'darwin)
       ...
       (defun ez/text-scale-reset ()
         (interactive)
         (text-scale-set 0))
       (bind-key "M-0" 'ez/text-scale-reset)
...
(defun ez/add-file-keybinding (key file &optional desc)
  (lexical-let ((key key)
                (file file)
                (desc desc))
...
(custom-set-variables '(org-agenda-files
                        '("~/org/main.org")))
(ez/add-file-keybinding "C-c m w" "~/org/main.org" "work.org")
...
(use-package org-capture
  ...
  (add-to-list 'org-capture-templates
               '("z"       ;`org-capture' binding + z
                 "ezzoueidi.com post"
                 entry
                 (file+olp "~/Personal/websites/ezzoueidi.com/content-org/ezzoueidi.com" "Ideas")
...
(quelpa '(ie-holidays :fetcher github :repo "ezzoueidi/ie-holidays"))
(require 'ie-holidays)
...
(use-package epa-file
  ...
  :config
  (setq epa-file-encrypt-to '("naeil@ezzoueidi.com"))
  :custom
...
(use-package org-crypt
  ...
  :custom
  (org-crypt-key "naeil@ezzoueidi.com"))
...
(defun ez/org-babel-tangle-async (file)
...
(defun ez/org-babel-tangle-current-buffer-async ()
  "Tangle current buffer asynchronously."
  (ez/org-babel-tangle-async (buffer-file-name)))
...
(defun ez/org-if-str (str &optional desc)
  (when (org-string-nw-p str)
    (or (org-string-nw-p desc) str)))

(defun ez/org-macro-hsapi-code (module &optional func desc)
  (org-link-make-string
   (concat "https://www.hammerspoon.org/docs/"
           (concat module (ez/org-if-str func (concat "#" func))))
   (or (org-string-nw-p desc)
       (format "=%s="
               (concat module
                       (ez/org-if-str func (concat "." func)))))))

(defun ez/org-macro-keys-code-outer (str)
  (mapconcat (lambda (s)
               (concat "~" s "~"))
             (split-string str)
             (concat (string ?\u200B) "+" (string ?\u200B))))
(defun ez/org-macro-keys-code-inner (str)
  (concat "~" (mapconcat (lambda (s)
                           (concat s))
                         (split-string str)
                         (concat (string ?\u200B) "-" (string ?\u200B)))
          "~"))
(defun ez/org-macro-keys-code (str)
  (ez/org-macro-keys-code-inner str))

(defun ez/org-macro-luadoc-code (func &optional section desc)
  (org-link-make-string
   (concat "https://www.lua.org/manual/5.3/manual.html#"
           (ez/org-if-str func section))
   (ez/org-if-str func desc)))

(defun ez/org-macro-luafun-code (func &optional desc)
  (org-link-make-string
   (concat "https://www.lua.org/manual/5.3/manual.html#"
           (concat "pdf-" func))
   (ez/org-if-str (concat "=" func "()=") desc)))
...
(use-package smart-mode-line
  ...
  (sml/replacer-regexp-list
   '(("^~/\\.emacs\\.d/elpa/"                            ":ELPA:")
     ...
     ("^:Dev:ezzoueidi.com/ezzoueidi.com/"                 ":ezzoueidi.org:"))))
...
(use-package hl-line
  ...
  :defer nil
  :config
  (defun ez/get-visual-line-range ()
    (let (b e)
      (save-excursion
      ...
      (cons b e)))
  (setq hl-line-range-function #'ez/get-visual-line-range)
  (global-hl-line-mode))
...
(defun ez/sp-enclose-next-sexp (num)
  (interactive "p")
...
(use-package easy-hugo
  :custom
  (easy-hugo-basedir "~/Personal/devel/ezzoueidi.com/ezzoueidi.com/")
  (easy-hugo-url "http://ezzoueidi.com/")
